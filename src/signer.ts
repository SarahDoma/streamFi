...
export class TransactionSigner implements Signer {
  ...
  async sign(tx: Transaction): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const cleanup = () => {
        ...
        this.pendingPromises.delete(cleanup);
      };
      ...
      this.pendingPromises.set(cleanup, reject);

      Promise.resolve()
        .then(async () => {
          if (this.isDestroyed) {
            cleanup();
            reject(new Error('TransactionSigner has been destroyed'));
            return;
          }
          if (cleanedUp) {
            return; // Already cleaned up by external call
          }
          if (this.walletAdapter) {
  ...
  publicKey(): string {
    if (!this.walletAdapter) {
      throw new Error('No wallet adapter configured');
    }
